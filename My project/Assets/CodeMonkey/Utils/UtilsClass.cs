...
namespace CodeMonkey.Utils
{
    ...
    public static class UtilsClass
    {
        ...
        public static void CreateWorldTextPopup(Transform parent, string text, Vector3 localPosition, int fontSize, Color color, Vector3 finalPopupPosition, float popupTime)
        {
            ...
            _ = FunctionUpdater.Create(delegate ()
            {
        ...
        private static readonly Color[] sequencialColors = new[] {
        ...
        public static Vector3 GetWorldPositionFromUI_Perspective(Vector3 screenPosition, Camera worldCamera)
        {
            ...
            _ = xy.Raycast(ray, out float distance);
        ...
        public static string GetMilliDots(long n)
        {
            ...
            {
                ret = ret[..(ret.Length - (i * 3) - (i - 1))] + "." + ret[(ret.Length - (i * 3) - (i - 1))..];
            }
            return ret;
        }
        ...
        {
            return n < 0 ? "-$" + GetMilliDots(Mathf.Abs(n)) : "$" + GetMilliDots(n);
        }
        ...
        public static Dictionary<TKey, TValue> LoadDictionaryJson<TKey, TValue>(string saveJson)
        {
            ...
            Dictionary<TKey, TValue> ret = new();
        ...
        {
            List<string> firstNameList = new(){"Gabe","Cliff","Tim","Ron","Jon","John","Mike","Seth","Alex","Steve","Chris","Will","Bill","James","Jim",
        ...
        {
            List<string> cityNameList = new(){"Alabama","New York","Old York","Bangkok","Lisbon","Vee","Agen","Agon","Ardok","Arbok",
        ...
        {
            GetTimeCharacterStrings(time, out string h0, out string h1, out string m0, out string m1, out string s0, out string s1, out string ms0, out string ms1, out string ms2);
            string h = h0 + h1;
            string m = m0 + m1;
            string s = s0 + s1;
            ...
            if (hours)
            {
                if (minutes)
                {
                    return seconds ? milliseconds ? h + ":" + m + ":" + s + "." + ms : h + ":" + m + ":" + s : h + ":" + m;
                }
                else
                {
                    return h;
            ...
            else
            {
                if (minutes)
                {
                    return seconds ? milliseconds ? m + ":" + s + "." + ms : m + ":" + s : m;
                }
                else
                {
                    return seconds ? milliseconds ? s + "." + ms : s : ms;
                }
            }
        }
        ...
        public static List<Vector2Int> GetPosXYListOblong(int width, int dropXamount, int increaseDropXamount)
        {
            ...
            List<Vector2Int> list = new(triangle);
